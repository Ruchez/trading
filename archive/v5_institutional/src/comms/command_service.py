...
class TelegramCommander:
    ...
    def __init__(self, bridge, risk_engine, notifier):
        ...
        self.pending_strategy_changes = {} # symbol -> new_mode
    ...
    def _process_message(self, message):
        ...
        if command == "/stop":
            ...
            # Syntax: /strategy <symbol> <mode>
            parts = text.split()
            if len(parts) >= 3:
                symbol = parts[1].upper()
                mode = parts[2].lower()
                
                # Basic validation: ensure symbol exists in MT5
                if mt5.symbol_info(symbol):
                    self.pending_strategy_changes[symbol] = mode
                    self.notifier.send_message(f"🔄 *STRATEGY SWITCH*\nRequested `{mode}` for `{symbol}`. Applying in next cycle...")
                else:
                    self.notifier.send_message(f"❌ *ERROR*\nSymbol `{symbol}` not found in MT5.")
            else:
                 self.notifier.send_message("ℹ️ *Usage*\n`/strategy <SYMBOL> <MODE>`\nModes: `intraday`, `scalp`")
